...
public class InfoAndroid {
	 ...
	 * @return Complete details about mobile firmware/hardware. This is the
	 */
		...
		try {
			...
			if (args[0].toString().equalsIgnoreCase("amond")) {
				if (args[1].toString().equalsIgnoreCase("-c") && args2) {
					...
					System.out
							.println("\t This tool will help to find the Android device information");
					System.out.println("");
					System.out.println("\t Console command usage:");
					System.out.println("\t amond [-c] <deviceNumber>");
					System.out
							.println("\t\t [*] options are mandatory with command");
					System.out
							.println("\t\t <*> options are Optional with command");
				}
			} else {
